...
const pages = {
  ...
  '/encrypted-file-sharing': {
    ...
    body: `${actions()}
      ...
      <section class="content-section"><h2>What encryption covers</h2><p>The file's contents are encrypted before upload using AES-256-GCM. Filenames and operational metadata are visible to the service; rename a file before uploading if its name is sensitive. Use the <a href="/security">security page</a> for the full model, or follow our <a href="/guides/share-password-protected-files">step-by-step sharing guide</a>.</p><p>Only sharing a short message? <a href="/encrypted-notes">Create an encrypted note instead</a>.</p></section>`,
  },
  '/encrypted-notes': {
    ...
    body: `${actions('Write a private note', '/share#note')}
      ...
      <section class="content-section"><h2>Limits and privacy</h2><p>A note can contain up to 10 MiB of encoded text; short messages are easier to use on mobile devices. It is stored as an encrypted note payload with the upload name <code>note.txt</code>.</p><p>Encryption protects the content, not a compromised browser or a link shared with the wrong person. QR links contain their own decryption key and should be treated as the secret itself.</p><p>Read <a href="/security">the security model</a>, <a href="/privacy">the privacy information</a>, or use <a href="/encrypted-file-sharing">file sharing</a> for an existing document.</p></section>`,
  },
  '/security': {
...
reiven --help</code></pre></section>
  ...
  '/guides/share-password-protected-files': {
    ...
    body: `<section class="content-section"><h2>1. Prepare the file and password</h2><p>Keep your original file. Reiven's hosted limit is 512 MiB of encrypted data including overhead, so leave room below that limit. Rename the file if its name reveals confidential information: the payload is encrypted, but the server receives the upload name.</p><p>Use a password manager to generate a long random password rather than a predictable phrase or a reused account password. Encryption cannot make a weak password resistant to offline guessing.</p></section>
